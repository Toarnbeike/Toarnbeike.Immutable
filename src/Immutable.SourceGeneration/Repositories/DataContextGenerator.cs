...
internal static class DataContextGenerator
{
    public static string Execute(IReadOnlyList<AggregateInfo> aggregates)
    {
        ...
        var interfaceDictionaries = string.Join("\n    ", aggregates
            .Select(aggregate => $$"""Dictionary<{{GetDictionaryCouple(aggregate)}}> {{aggregate.PluralName}} { get; }"""));
        
        var classDictionaries = string.Join("\n    ", aggregates
            .Select(aggregate => $$"""public Dictionary<{{GetDictionaryCouple(aggregate)}}> {{aggregate.PluralName}} { get; private set; } = new(); """));
        
        return $$"""
                 // <auto-generated />
                 // generator: Toarnbeike.Immutable.SourceGeneration.Repositories.DataContextGenerator
                 #nullable enable
